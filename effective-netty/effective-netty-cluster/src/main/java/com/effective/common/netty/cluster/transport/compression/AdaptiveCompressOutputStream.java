...
public class AdaptiveCompressOutputStream extends OutputStream implements Finishable {
     ...
     */
    public AdaptiveCompressOutputStream(final ByteBuf buffer, final Compression compression) {
     ...
     * @throws IllegalArgumentException
     */
    public AdaptiveCompressOutputStream(final ByteBuf buffer, final Compression compression, final int threshold) {
        if (buffer == null) {
            throw new NullPointerException("ByteBuf can not be null.");
        } else if (compression == null) {
            throw new NullPointerException("Compression function can not be null.");
        } else if (threshold <= 0) {
    ...
    protected void compress() throws IOException {
        if (out == null) {
            ...
            if (size > 0) {
                ...
                //转换成压缩流，再次写入数据
                out = compression.compress(new ByteBufOutputStream(buffer));
                out.write(bytes);
            } else {
                out = compression.compress(new ByteBufOutputStream(buffer));
            }
